...
bool Mesh::intersectFace(const Ray& ray, Hit& hit, int faceId) const
{
    ...
    float tf= 1/(p.dot(e1)) * q.dot(e2);
    float u= 1/(p.dot(e1)) * p.dot(t);
    float v= 1/(p.dot(e1)) * q.dot(d);
    ...
    if(u+v<0 || u+v>1)
      ...
      hit.setT(tf);
      //Normal3f norm= e1.cross(e2).normalized();
      Normal3f norm=a.normal*u + b.normal*v + c.normal *(1-u-v);
      norm.normalize();
      hit.setNormal(norm);
      hit.setShape(this);
      Vector2f uv(u,v);
...
bool Mesh::intersect(const Ray& ray, Hit& hit) const
{
    ...
    if(m_BVH){
      ...
      for(size_t i=0; i<m_faces.size();i++){
        intersectFace(ray,hitTmp,i);
        if (hitTmp.foundIntersection()){
          if(hitTmp.t()<hit.t()){
            hit.setT(hitTmp.t());
