...
{
    for(Vertex& vertex:mVertices)vertex.normal = Vector3f(0,0,0);
    for(unsigned int i = 0; i <  mFaces.size(); i++)
    {
        Vector3i face = mFaces[i];
        Vector3f e1 = mVertices[face.x()].position - mVertices[face.z()].position;
        Vector3f e2 = mVertices[face.y()].position - mVertices[face.z()].position;

        mVertices[face.x()].normal+= e1.cross(e2);
        mVertices[face.y()].normal+= e1.cross(e2);
        mVertices[face.z()].normal+= e1.cross(e2);
